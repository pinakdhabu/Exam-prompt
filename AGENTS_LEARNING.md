...
> fix a bug caused by missing context, or learn something future agents should know — record it here.
>
...
- **Number freshness**: After any count change (skills, subjects, universities), grep the whole repo for the old number.
- **Infrastructure discovery**: Always check `infrastructure/`, `bin/`, `scripts/` for tools not yet wired into agent configs.
- **JSON validation**: Run `node -e "require('./path.json')"` after editing JSON files to catch syntax errors immediately.
