...
description: >
  Pre-optimized sample answers and prompts for ANY subject at ANY university worldwide. Contains a
  Subject Template Framework covering Engineering, Science, Management, Law, Medical, Computer
  Science, Mathematics, and Humanities disciplines. Includes SPPU Computer Engineering examples as
  one reference implementation among many. Covers 2-mark, 4-mark, 6-mark, 10-mark, and 15-mark
  question patterns with time-budgeted responses.
---
...
2. **Linear Requirement**: Only applicable to circuits obeying Ohm's Law — resistors, capacitors,
   inductors.
...
5. **Generalization**: For any closed surface,
   $\oint \vec{E} \cdot d\vec{A} = \frac{Q_{\text{enc}}}{\varepsilon_0}$.
6. **Significance**: Relates electric field distribution to charge distribution; fundamental to
   Maxwell's equations.
...
5. **Rivalry Among Existing Competitors**: Intensity of competition (price wars, advertising,
   innovation).
...
1. **Product**: Smartphone with AI camera, 5G, 5000mAh battery, OLED display. Unique selling point:
   AI-powered photography at mid-range price.
2. **Price**: Penetration pricing strategy — ₹24,999 for initial 3 months then ₹29,999. Competitors
   at ₹35,000+ for similar specs.
3. **Place**: Multi-channel distribution — online via Amazon/Flipkart, offline through 500+
   exclusive stores and partnered retail chains.
4. **Promotion**: Digital-first campaign using YouTube influencers, Instagram ads, and in-store demo
...
3. **Reasonable Classification**: Article 14 permits classification if (a) based on intelligible
   differentia, (b) differentia has rational nexus to legislative objective.
4. **Landmark Case**: _State of West Bengal v. Anwar Ali Sarkar_ (1952) established that arbitrary
...
2. **From Promisee or Third Party**: Consideration may be provided by the promisee or any other
   person.
3. **Past, Present, or Future**: Consideration may be executory (future), executed (present), or
   past.
4. **Need Not Be Adequate**: Law only requires presence of consideration, not adequacy (_Thomas v.
   Thomas_, 1842).
5. **Must Be Real**: Physical impossibility or illusory promises do not amount to valid
   consideration.
...
1. **Pericardium**: Double-walled fibrous sac enclosing the heart — outer fibrous and inner serous
   layers with pericardial fluid.
2. **Chambers**: Right atrium (receives deoxygenated blood via SVC/IVC), Right ventricle (pumps to
   lungs), Left atrium (receives oxygenated blood from pulmonary veins), Left ventricle (pumps to
   aorta — thickest wall).
3. **Valves**: Tricuspid (right AV), Mitral/bicuspid (left AV), Pulmonary (right ventricular
...
1. **Atherosclerosis**: Chronic inflammation leads to lipid-laden plaque formation in coronary
   arteries.
...
4. **Ischemia**: Complete occlusion stops oxygen supply to downstream myocardium within 20–40
   minutes.
...
1. **Algorithm**: Compare target with middle element; if equal return index; if less, search left
   half; if greater, search right half.
2. **Recurrence Relation**: $T(n) = T(n/2) + O(1)$
3. **Analysis**: Each comparison halves the array — after $k$ comparisons, array size is $n/2^k$.
...
1. **Nationalism**: Pan-Slavism in Balkans threatened Austria-Hungary; French revanchism over
   Alsace-Lorraine.
2. **Imperial Rivalries**: Colonial competition between Britain, France, and Germany in Africa and
   Asia.
3. **Alliance System**: Triple Entente (UK, France, Russia) vs. Triple Alliance (Germany,
   Austria-Hungary, Italy) turned bilateral disputes into continental war.
4. **Militarism**: Arms race (especially Anglo-German naval race) and Schlieffen Plan forced rapid
   mobilization.
5. **Immediate Trigger**: Assassination of Archduke Franz Ferdinand (June 28, 1914) activated
...
1. **Justice Problem**: Could justify punishing an innocent person if it maximizes overall happiness
   (e.g., railroading a suspect to prevent riots).
2. **Measurement Problem**: Pleasure/pain is subjective — impossible to quantify or compare across
   individuals.
3. **Demandingness**: Requires constant impartial calculation, ignoring personal relationships and
   commitments.
...
4. **Cardinality**: Specifies number of instances of one entity associated with another (e.g.,
   one-to-many).
...
1. **Identify the subject's discipline** from the Subject Template Framework (Section: Subject
   Template Framework).
2. **Determine the marks** and locate the corresponding time budget in Time-Budgeted Answer
   Framework.
3. **Select the discipline template** and follow its answer anatomy (Concept → Formula → Example or
   Principle → Case → Application, etc.).
4. **Reference the sample answers** in the relevant template section for structural guidance.
5. **For SPPU-specific questions**, refer to the Reference Implementation section which maps
