...
> Pattern subject. Predictions based on official syllabus + cross-reference with TE-level DBMS PYQs
> (which share ~60% content overlap).
...
1. Design an ER diagram for a Library Management System. Convert it to relational schema with all
   keys identified.
...
4. Check whether the following schedule is conflict serializable: (provide schedule). Draw
   precedence graph.
...
- **Normalization forms:** **"1-2-3-B"** = 1NF (atomic), 2NF (no partial), 3NF (no transitive), BCNF
